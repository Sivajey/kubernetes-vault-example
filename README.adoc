...
* A deployment of Vault configured as described in the [`kubernetes-vault documentation`](https://github.com/travelaudience/kubernetes-vault).
* It is assumed that:
...
$ vault write sys/policy/time-server-credentials-ro \
    rules="@time-server-credentials-ro.hcl"
