...
* It is assumed that:
** Vault is reachable at https://vault.travelaudience.com.
** An instance of the
   https://www.vaultproject.io/docs/auth/kubernetes.html[Kubernetes authentication backend]
   has been mounted as `training-235717` and is properly configured to access to the
...
Since one also wants to establish mutual TLS authentication, one must also
configure the `pki` secret backend. One will need two roles — one for the server
and one for the client:

[source,bash]
----
$ ALLOWED_DOMAINS=(
    time-server.default.svc.cluster.local
    time-server.default
    time-server
)
...
Now one needs to create a policy, named `time-server`, that grants access to the
paths necessary to read the credentials and request the certificates:
...
`training-235717-gke-1` authentication backend:
...
- Authentication is also only allowed in the `default` namespace.
...
the `default` namespace, one must run
...
At this point, inspecting pods in the `default` namespace
...
Fetch logs from the `time-server-0` pod:
[source,bash]
----
...
Now, fetch the logs from the `time-client-0` pod:
[source,bash]
----
...
`02-using-secrets-from-vault/01-configmap.yaml`, should be similar to the
following:
...
data:
  config.yaml: |
    address: https://vault.travelaudience.com
    auth:
      type: kubernetes
      backend: training-235717-gke-1
      data:
...
[NOTE]
====
- One must replace `vault.travelaudience.com` with the domain where Vault can be reached.
- The type of authentication backend being used. This has the fixed value `kubernetes`.
- One must replace `training-235717-gke-1` with the actual path to the mount in Vault.
...
`02-using-secrets-from-vault/02-time-server.yaml` and
`02-using-secrets-from-vault/03-time-client.yaml`. One should carefully analyze
...
`default` namespace to make sure there are no errors:
...
[source,bash]
----
$ kubectl logs --container kubernetes-vault-client time-server-0
...
$ kubectl logs --container kubernetes-vault-client time-client-0
...
safe#passw0rd!
$ kubectl exec time-client-0 -- cat /secret/username
time-client
$ kubectl exec time-client-0 -- cat /secret/password
safe#passw0rd!
----
      ...
      data:
        pki:
          - mountName: intermediate-ca
            role: time-server
            cn: time-server.default.svc.cluster.local
            sans:
            - time-server.default
            - time-server
            - localhost
...
[source,bash]
----
$ kubectl cp time-client-0:/secret/chain.pem ./chain.pem
tar: removing leading '/' from member names

$ kubectl cp time-client-0:/secret/crt.pem ./crt.pem
tar: removing leading '/' from member names

$ kubectl cp time-client-0:/secret/crt.pem ./key.pem
tar: removing leading '/' from member names
----
...
[source,bash]
----
$ kubectl port-forward time-server-0 8443:443
